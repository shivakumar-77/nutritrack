...
interface Food { name:string; cal:number; protein:number; carb:number; fat:number; fiber:number; unit:string; baseQty:number; description?:string }
interface MacroResult extends Food { qty:number }
const MEAL_TYPES = ['breakfast','lunch','dinner','snack','other']
const today = () => new Date().toISOString().slice(0,10)

function scaleFood(food:Food, qty:number): MacroResult {
...
export default function LogPage() {
  ...
  const preview = selected ? scaleFood(selected, qty) : null
  ...
  async function scanPhoto(e:React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = async ev => {
      ...
      try {
        ...
        const data = await res.json()
        if (data.result) {
          const food: Food = { ...data.result, baseQty: data.result.qty }
          setSelected(food); setQty(data.result.qty)
  ...
  const MacroPreviewCards = ({ p }: { p: MacroResult }) => (
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8,marginBottom:16}}>
      {[{l:'Calories',v:p.cal,u:'kcal',c:'#4ade80'},{l:'Protein',v:p.protein,u:'g',c:'#60a5fa'},{l:'Carbs',v:p.carb,u:'g',c:'#fbbf24'},{l:'Fat',v:p.fat,u:'g',c:'#f87171'}].map(m=>(
  ...
  const QtyPicker = ({ food }: { food: Food }) => (
    <div style={{marginBottom:16}}>
      <label style={{fontSize:12,color:'var(--muted)',display:'block',marginBottom:8}}>Adjust quantity ({food.unit})</label>
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        <button onClick={()=>setQty(q=>Math.max(food.unit==='piece'||food.unit==='scoop'?1:10, q-(food.unit==='piece'||food.unit==='scoop'?1:10)))}
          style={{width:44,height:44,borderRadius:12,background:'var(--card2)',border:'1px solid var(--border)',color:'var(--text)',fontSize:22,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>−</button>
        <input type="number" value={qty} onChange={e=>setQty(Math.max(0,parseFloat(e.target.value)||0))}
          style={{textAlign:'center',fontWeight:600,fontSize:20,flex:1}}/>
      ...
      <div style={{display:'flex',gap:6,marginTop:8,flexWrap:'wrap'}}>
        {(food.unit==='g'||food.unit==='ml'?[50,100,150,200,250,300]:[1,2,3,4,5]).map(v=>(
          <button key={v} onClick={()=>setQty(v)}
            style={{padding:'4px 12px',borderRadius:99,fontSize:12,border:`1px solid ${qty===v?'var(--green)':'var(--border)'}`,background:qty===v?'rgba(74,222,128,0.15)':'transparent',color:qty===v?'var(--green)':'var(--muted)',cursor:'pointer'}}>
            {v}{food.unit}
          </button>
        ))}
      </div>
  ...
  const MealTypePicker = () => (
    <div style={{marginBottom:16}}>
      <label style={{fontSize:12,color:'var(--muted)',display:'block',marginBottom:8}}>Add to meal</label>
      <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
        {MEAL_TYPES.map(m=>(
          <button key={m} onClick={()=>setMealType(m)}
            className={mealType===m?'btn btn-primary':'btn btn-ghost'}
            style={{padding:'6px 14px',fontSize:12,borderRadius:99}}>
  ...
  return (
    <div className="page" style={{paddingTop:24}}>
      ...
      <div style={{display:'flex',gap:8,marginBottom:20,overflowX:'auto',paddingBottom:4}}>
        {(['search','scan','barcode','manual'] as const).map(t=>(
          <button key={t} onClick={()=>setTab(t)}
            ...
            style={{padding:'8px 16px',fontSize:13,borderRadius:99,whiteSpace:'nowrap'}}>
            ...
            <div style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,overflow:'hidden',marginBottom:16}}>
              {results.map((r,i)=>(
                <button key={i} onClick={()=>selectFood(r)}
                  style={{width:'100%',padding:'14px 16px',textAlign:'left',background:'none',border:'none',borderBottom:i<results.length-1?'1px solid var(--border)':'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'space-between'}}
        ...
        <div>
          ...
          {!photoPreview && !selected && (
            <div style={{textAlign:'center',padding:'24px 0'}}>
              ...
              <p style={{fontWeight:500,fontSize:16,marginBottom:8}}>AI Food Scanner</p>
              <p style={{color:'var(--muted)',fontSize:13,marginBottom:24,lineHeight:1.6}}>Take a photo of your meal — AI will identify the food and calculate macros automatically</p>
              <button className="btn btn-primary pulse-green" style={{width:'100%',padding:'16px',fontSize:15,fontWeight:600}} onClick={()=>fileRef.current?.click()}>
                Take photo of food
              </button>
            </div>
          )}
          ...
          {scanning && (
            <div style={{textAlign:'center',padding:'24px 0'}}>
              {photoPreview && <img src={photoPreview} alt="Food" style={{width:'100%',borderRadius:16,marginBottom:20,maxHeight:280,objectFit:'cover'}}/>}
              <div style={{width:32,height:32,borderRadius:'50%',border:'2px solid var(--green)',borderTopColor:'transparent',animation:'spin 0.7s linear infinite',margin:'0 auto 12px'}}/>
          ...
          {scanError && !scanning && (
            <div>
              {photoPreview && <img src={photoPreview} alt="Food" style={{width:'100%',borderRadius:16,marginBottom:16,maxHeight:280,objectFit:'cover'}}/>}
              <div style={{background:'rgba(248,113,113,0.1)',border:'1px solid rgba(248,113,113,0.3)',borderRadius:12,padding:'12px 14px',marginBottom:16}}>
                <p style={{color:'var(--red)',fontSize:13}}>{scanError}</p>
              </div>
              <button className="btn btn-primary" style={{width:'100%',marginBottom:10}} onClick={()=>{setPhotoPreview(null);setScanError('');fileRef.current?.click()}}>Try again</button>
            </div>
          )}

          {selected && preview && !scanning && tab==='scan' && (
            <div className="slide-up">
              {photoPreview && <img src={photoPreview} alt="Food" style={{width:'100%',borderRadius:16,marginBottom:16,maxHeight:240,objectFit:'cover'}}/>}
              {selected.description && <p style={{fontSize:13,color:'var(--muted)',marginBottom:16,lineHeight:1.6}}>{(selected as any).description}</p>}
              <div className="card">
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
                  <div>
                    <div style={{fontWeight:600,fontSize:16}}>{selected.name}</div>
                    <div style={{fontSize:12,color:'var(--green)',marginTop:2}}>AI identified</div>
                  </div>
                  <button onClick={()=>{setSelected(null);setPhotoPreview(null)}} style={{background:'none',border:'none',color:'var(--muted)',cursor:'pointer',fontSize:20}}>×</button>
                </div>
                <MacroPreviewCards p={preview}/>
                <QtyPicker food={selected}/>
                <MealTypePicker/>
      ...
      {/* BARCODE */}
      {tab==='barcode' && (
        <BarcodeScanner onResult={f=>{
          const food: Food = {...f, baseQty: f.qty}
          setSelected(food); setQty(f.qty); setTab('search')
        }}/>
      )}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
            {[{l:'Calories (kcal)',k:'cal',c:'#4ade80'},{l:'Protein (g)',k:'protein',c:'#60a5fa'},{l:'Carbs (g)',k:'carb',c:'#fbbf24'},{l:'Fat (g)',k:'fat',c:'#f87171'},{l:'Fiber (g)',k:'fiber',c:'#a78bfa'}].map(f=>(
              <div key={f.k}><label style={{fontSize:12,color:f.c,display:'block',marginBottom:6}}>{f.l}</label>
                <input type="number" placeholder="0" value={manual[f.k as keyof typeof manual]} onChange={e=>setManual(p=>({...p,[f.k]:e.target.value}))}/></div>
            ))}
          </div>
          <MealTypePicker/>
          <button className="btn btn-primary" style={{padding:'14px',fontSize:15,fontWeight:600}} onClick={logManual} disabled={saving}>
